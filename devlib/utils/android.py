...
from devlib.connection import (ConnectionBase, AdbBackgroundCommand,
                               PopenTransferHandle, TransferManager)

from typing import (Optional, TYPE_CHECKING, cast, Tuple, Union,
                    List, DefaultDict, Pattern, Dict, Iterator,
                    Match, Callable, Generator)
...
MAX_ATTEMPTS: int = 5
AM_START_ERROR: Pattern[str] = re.compile(r"Error: Activity.*")
AAPT_BADGING_OUTPUT: Pattern[str] = re.compile(r"no dump ((file)|(apk)) specified", re.IGNORECASE)
...
class ApkInfo(object):
    ...
    """
    version_regex: Pattern[str] = re.compile(r"name='(?P<name>[^']+)' versionCode='(?P<vcode>[^']+)' versionName='(?P<vname>[^']+)'")
    name_regex: Pattern[str] = re.compile(r"name='(?P<name>[^']+)'")
    permission_regex: Pattern[str] = re.compile(r"name='(?P<permission>[^']+)'")
    activity_regex: Pattern[str] = re.compile(r'\s*A:\s*android:name\(0x\d+\)=".(?P<name>\w+)"')
    ...
    def parse(self, apk_path: str) -> None:
        ...
        for line in output.split('\n'):
            if line.startswith('application-label:'):
                ...
                match: Optional[Match[str]] = self.version_regex.search(line)
                if match:
...
class AdbConnection(ConnectionBase):
    ...
    def background(self, command: 'SubprocessCommand', stdout: int = subprocess.PIPE,
        ...
        :type command: SubprocessCommand
        :param stdout: File descriptor or special value (e.g., subprocess.PIPE) for stdout.
        :type stdout: int
...
def grant_app_permissions(target: 'AndroidTarget', package: str) -> None:
    ...
    permissions: Optional[Match[str]] = re.search(
...
class _AndroidEnvironment:
    ...
    def get_env(self, name: Android_Env_TypeKeys) -> Optional[Union[str, int]]:
        ...
        :type name: Android_Env_TypeKeys
